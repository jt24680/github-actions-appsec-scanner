...
Includes a custom **GitHub Actions workflow exploit guard** that checks for 100+ security exploits and anti-patterns in GitHub Actions YAMLs that other scanners don't search for. If it finds any issues it will report the "why" behind the error and suggest / require a fix
...
[↑ top](#github-actions-appsec-scanner)
...
See [`examples/pr.yaml`](examples/pr.yaml) and
[`examples/full-scan.yaml`](examples/full-scan.yaml) for ready-to-use examples.
