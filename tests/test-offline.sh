...
run_test "example pr workflow exists" bash -c "
  [ -f '$REPO_ROOT/examples/pr.yaml' ]
"

run_test "example pr uses the public reusable API with PR comment" bash -c "
  grep -Fq 'uses: your-org/github-actions-appsec-scanner/.github/workflows/orchestrator-reusable.yaml@main' '$REPO_ROOT/examples/pr.yaml' || exit 1
  grep -Fq 'enable_pr_comment: true' '$REPO_ROOT/examples/pr.yaml'
"

run_test "examples directory contains only supported workflow examples" bash -c "
  actual=\$(cd '$REPO_ROOT/examples' && ls -1 | sort)
  expected=\$'full-scan.yaml\npr.yaml'
  [ \"\$actual\" = \"\$expected\" ]
"

run_test "example full-scan workflow exists" bash -c "
  [ -f '$REPO_ROOT/examples/full-scan.yaml' ]
"

run_test "example full-scan uses the public reusable API with code scanning upload" bash -c "
  grep -Fq 'uses: your-org/github-actions-appsec-scanner/.github/workflows/orchestrator-reusable.yaml@main' '$REPO_ROOT/examples/full-scan.yaml' || exit 1
  grep -Fq 'enable_code_scanning_upload: true' '$REPO_ROOT/examples/full-scan.yaml'
"
...
run_test "exploit-guards action exposes fail-on-findings input" \
  assert_literal_in_file "$EG_ACTION" "fail-on-findings:"

run_test "exploit-guards action hardens output writes against symlink traps" bash -c "
  grep -Fq 'if [ -L exploit-guards.txt ]; then' '$EG_ACTION' || exit 1
  grep -Fq 'if [ -L exploit-guards.sarif ]; then' '$EG_ACTION' || exit 1
  grep -Fq 'mv -f -- \"\$txt_tmp\" exploit-guards.txt' '$EG_ACTION' || exit 1
  grep -Fq 'mv -f -- \"\$sarif_tmp\" exploit-guards.sarif' '$EG_ACTION'
"

run_test "exploit-guards action stages temporary outputs in safe temp directories" bash -c "
  grep -Fq 'mktemp \"${RUNNER_TEMP:-${TMPDIR:-/tmp}}/.exploit-guards.txt.XXXXXX\"' '$EG_ACTION' || exit 1
  grep -Fq 'mktemp \"${RUNNER_TEMP:-${TMPDIR:-/tmp}}/.exploit-guards.sarif.XXXXXX\"' '$EG_ACTION' || exit 1
"

run_test "exploit-guards action has configurable findings gating" bash -c "
  grep -Fq 'FAIL_ON_FINDINGS: \${{ inputs.fail-on-findings }}' '$EG_ACTION' || exit 1
  grep -Fq 'exploit-guards reported findings and fail-on-findings is enabled.' '$EG_ACTION'
"

run_test "exploit-guards action hardens python interpreter invocations" bash -c "
  grep -Fq 'python_bin=\"\$(command -v python3)\"' '$EG_ACTION' || exit 1
  grep -Fq 'PYTHONPATH=\"\" PYTHONNOUSERSITE=1 \"\$python_bin\" -I -P' '$EG_ACTION' || exit 1
"
...
run_test "exploit-guards emits runtime failure summary and fail-closed exit path" bash -c "
  grep -Fq 'Runtime failures: \$runtime_fail' '$EG_SCRIPT' || exit 1
  grep -Fq 'if [ \"\$runtime_fail\" -ne 0 ]; then' '$EG_SCRIPT'
"
