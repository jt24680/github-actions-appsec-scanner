...
make_log_file() {
  ...
  if [ -n "$LOG_DIR" ]; then
    path="$(mktemp "$LOG_DIR/${safe}.XXXXXX.log")"
  else
    path="$(mktemp "${TMPDIR:-/tmp}/appsec-act-${safe}.XXXXXX.log")"
    TMP_PATHS+=("$path")
  fi
...
make_result_file() {
  ...
  path="$(mktemp "${TMPDIR:-/tmp}/appsec-act-${safe}.XXXXXX.result")"
  TMP_PATHS+=("$path")
  echo "$path"
}
...
disable_general_job_for_act() {
  ...
  "$PYTHON_BIN" - "$dir/.github/workflows/_orchestrator-ci.yaml" <<'PY'
from pathlib import Path
import sys
